...
export const addProgram = async (req, res) => {
  try {
    const { title, conductedBy } = req.body;

    console.log(title, conductedBy);

    // ✅ validation
    if (!title || !conductedBy) {
      return res.status(400).json({
        message: 'Program title and conductedBy are required'
      });
    }
