...
const fetchPrograms = async () => {
    try {
        const res = await axios.get("/unit/programs/list");
 ...
 const handleProgramFormChange = (e) => {
    ...
    //     const {   name, customName, date, description, newFiles, existingImagesToKeep, imagesToDelete } = submissionData;
    //     const finalProgramName = name === "__custom__"? customName : name;
    //     if(finalProgramName){name = finalProgramName ; }
   ...
   const handleProgramSubmit = async (submissionData) => {
    ...
    if (!finalProgramName || !date) {
        dispatch(showNotification({
            type: 'error',
            message: 'Program name and date are required.'
        }));
        return;
    }
    ...
    formData.append('name', finalProgramName);
    formData.append('date', date);
    formData.append('description', description);

    formData.append(
        "existingImages",
        JSON.stringify(submissionData.existingImages || [])
    );

    // delete image keys
    formData.append(
        "imagesToDelete",
        JSON.stringify(submissionData.imagesToDelete || [])
    );
    ...
    try {
        ...
        if (editingProgram) {
            ...
            response = await axios.put(
                `/unit/programs/${editingProgram._id}`,
                formData,
                {
                    headers: {
            ...
            <AnimatePresence>
                {isAddingProgram && (
                    <AddProgramModal
                        ...
                        onFormChange={handleProgramFormChange}
